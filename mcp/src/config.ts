...
export const config = {
    baseUrl: normaliseBaseUrl(process.env.SOLIDTIME_API_URL ?? 'https://app.solidtime.io/api'),
    apiToken: requireEnv('SOLIDTIME_API_TOKEN'),
    /** Optional default so agents can omit organization_id on every call. */
    defaultOrganizationId: process.env.SOLIDTIME_ORGANIZATION_ID?.trim() || undefined,
